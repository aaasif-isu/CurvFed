...
def build_distrib_for_round(smashed_dir, distrib_dir):
    for fname in sorted(os.listdir(smashed_dir)):
        if fname.endswith(".pt"):
            client_id = fname.replace("client_", "").replace(".pt", "")
            x = torch.load(os.path.join(smashed_dir, fname), map_location="cpu").float()
            if x.dim() > 2:
                x = x.view(x.size(0), -1)
            torch.save(x, os.path.join(distrib_dir, f"client{client_id}.pt"))
...
def cluster_clients_for_round(emd, out_dir, n_clusters=3):
    try:
    ...
    for client_id, cluster_id in enumerate(labels):
        cluster_groups.setdefault(int(cluster_id), []).append(client_id)

    with open(os.path.join(out_dir, "cluster_summary.txt"), "w") as f:
        f.write("Cluster assignments by client\n")
        for client_id, cluster_id in enumerate(labels):
            f.write(f"Client {client_id} -> Cluster {cluster_id}\n")

        f.write("\nCluster groups\n")
        for cluster_id, clients in sorted(cluster_groups.items()):
            f.write(f"Cluster {cluster_id}: {clients}\n")

    np.savetxt(os.path.join(out_dir, "client_clusters.txt"), labels, fmt="%d")

    return labels, cluster_groups


def clusterwise_fedavg(client_weights_by_id, cluster_groups):
    cluster_models = []

    for cluster_id, client_ids in sorted(cluster_groups.items()):
        weights = [client_weights_by_id[cid] for cid in client_ids]
        cluster_models.append(FedAvg(weights))

    return FedAvg(cluster_models), cluster_models
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                with torch.no_grad():
                    ...
                    if smashed_dir is not None:
                        os.makedirs(smashed_dir, exist_ok=True)
                        torch.save(_fx[_idx], os.path.join(smashed_dir, f"client_{self.idx}.pt"))
...
for global_round in range(epochs):
    ...
    for idx in idxs_users:
        ...
        client_weights_by_id[idx] = copy.deepcopy(w_client)
    ...
    build_distrib_for_round(smashed_dir, distrib_dir)

    emd = compute_emd_for_round(distrib_dir, out_dir)

    compute_ricci_for_round(emd, out_dir)

    labels, cluster_groups = cluster_clients_for_round(
        emd,
        out_dir,
        n_clusters=3
    )

    print("Cluster groups:", cluster_groups)

    os.makedirs("outputs/rounds", exist_ok=True)

    # Save cluster info
    with open(f"outputs/rounds/round_{global_round + 1:03d}_clusters.json", "w") as f:
        json.dump(cluster_groups, f, indent=4)
    
    # Save EMD matrix
    np.save(f"outputs/rounds/round_{global_round + 1:03d}_emd.npy", emd)

    w_glob_client, cluster_models_client = clusterwise_fedavg(
        client_weights_by_id,
        cluster_groups
    )
    for cidx, cmodel in enumerate(cluster_models_client):
        torch.save(
            cmodel,
            os.path.join(out_dir, f"client_cluster_model_{cidx}.pth")
    )
    ...
    torch.save(
        w_glob_client,
        os.path.join(out_dir, f"client_global_model_round_{global_round + 1:03d}.pth")
    )

    torch.save(
        w_glob_server,
        os.path.join(out_dir, f"server_global_model_round_{global_round + 1:03d}.pth")
    )    
